...
* `IsLocalRing.ringKrullDim_quotient_succ_eq_of_nonZeroDivisor`: If `x` is a non-unit,
  non-zerodivisor then `ringKrullDim R = 1 + ringKrullDim R ⧸ x`
...
/--Definition: The Krull dimension of R cast as a natural number. If R has infinite Krull dimension
or is the zero ring then finRingKrullDim R = 0 -/
noncomputable def finRingKrullDim (R : Type*) [CommRing R] : ℕ :=
  ENat.toNat (WithBot.unbotD 0 (ringKrullDim R))

/- A noetherian local ring has finite Krull dimension.-/
...
/--Lemma: If `R` is a ring with finite Krull dimension, then `finRingKrullDim R = ringKrullDim R`.-/
...
/--Lemma: If `R` is a ring, `I ⊆ R` is a prime ideal, and `P` is a minimal prime ideal of `R`, then
`I ⊆ P` implies that `I=P`. -/
lemma le_minimalPrimes
{R : Type*} [CommRing R] {I : Ideal R} [I.IsPrime] {P : Ideal R} (hP : P ∈ minimalPrimes R)
    (hI : I ≤ P) : I = P := by
...
/--Lemma: If `(R,m)` is noetherian local ring, then there exists a subset `s ⊆ R` of cardinality
at most `finRingKrullDim R` such that `m` is a minimal prime over `(Ideal.span s)` -/
theorem IsLocalRing.exists_finset_card_eq_ringKrullDim (R : Type*) [CommRing R] [IsLocalRing R]
    [IsNoetherianRing R] : ∃ s : Finset R, s.card ≤ finRingKrullDim R ∧
    (maxl R) ∈ (Ideal.span s).minimalPrimes := by
...
/-- Theorem: If `(R,m)` is a noetherian local ring and `x ∈ m` then
`ringKrullDim R ≤ 1 + ringKrullDim R ⧸ x`. In other words, when you
quotient out by a non-unit, the dimension drops by at most 1 -/
theorem IsLocalRing.Quotient_singleton_ringKrullDim [IsLocalRing R] [IsNoetherianRing R] (x : R)
  ...
  have im : Ideal.map (Ideal.Quotient.mk (Ideal.span {x})) (Ideal.span S) = Ideal.span s := by
    have imS : (Ideal.Quotient.mk (Ideal.span {x}))'' S = s := by
      ...
      ext x_1 : 1
      simp_all only [Set.mem_image, Finset.mem_coe, exists_exists_and_eq_and, Ideal.Quotient.mk_out,
        exists_eq_right]
    rw[Ideal.map_span, imS]
  have comapmap := Ideal.comap_map_of_surjective' (Ideal.Quotient.mk (Ideal.span {x}))
  ...
  have maxmin : (maxl R) ∈ (span S').minimalPrimes := by
    ...
    exact this
  have KDRle := (Ideal.height_le_spanRank_toENat_of_mem_minimal_primes (span (S' : Set R))
    (maxl R)) maxmin
  have maxKDR : (maxl R).height = ringKrullDim R := maximalIdeal_height_eq_ringKrullDim
  have spanrk := @Submodule.spanFinrank_span_le_ncard_of_finite R R _ _ _ S'
...
/--Lemma: If `(R,m)` is a noetherian local ring and `x ∈ m \ m^2`, then
`spanFinrank(m) = spanFinrank(m/xm) + 1`.-/
...
/--Lemma: If `(R,m)` is a noetherian local ring, then `ringKrullDim R ≤ spanFinrank(m)`.-/
theorem IsLocalRing.ringKrullDim_le_spanFinrank (R : Type*) [CommRing R] [IsLocalRing R]
    [IsNoetherianRing R] : ringKrullDim R ≤ Submodule.spanFinrank (maxl R) := by
...
/--Theorem: The embedding dimension of `R` is an upper bound on the Krull Dimension of `R`. -/
theorem IsLocalRing.ringKrullDim_le_embDim (R : Type*) [CommRing R] [IsLocalRing R]
    [IsNoetherianRing R] : ringKrullDim R ≤ embDim R := by
  rw[IsLocalRing.embDim_eq_spanFinrank_maximalIdeal]
  exact IsLocalRing.ringKrullDim_le_spanFinrank R

/--Theorem: If `x` is a non-unit, non-zerodivisor then `ringKrullDim R = 1 + ringKrullDim R ⧸ x` -/
theorem IsLocalRing.ringKrullDim_quotient_succ_eq_of_nonZeroDivisor
    {R : Type*} [CommRing R] [IsLocalRing R] [IsNoetherianRing R] {r : R}
    (hr : r ∈ nonZeroDivisors R) (hr' : ¬IsUnit r) :
